...
def predict_all(learner):
    ...
    with torch.no_grad():
        for X_test, sizes_test in tqdm_notebook(learner.dl(DatasetType.Test)):
            ...
            preds.append(learner.model(X_test).to('cpu'))
...
def predict_TTA(learner, img, size, rotations):
    ...
    for angle in rotations:
        ...
        rot = TF.to_tensor(rot).unsqueeze(0).cuda()
        rot_flipped = TF.rotate(flipped, angle)
        rot_flipped = TF.to_tensor(rot_flipped).unsqueeze(0).cuda()
...
def create_submission(preds, sizes, test_ids, resize=False):
    ...
    sub_file = os.path.join(sub_path,
                            f'sub_dsbowl_pt_{getNextFilePath(sub_path)}.csv')
