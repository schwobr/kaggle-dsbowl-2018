...
# PATHS
PROJECT_PATH = Path('/work/stages/schwob/data-science-bowl-2018/kaggle-dsbowl-2018/')
...
def mean_iou(y_pred, y_true, smooth=1e-6):
    ...
    y_pred = torch.sigmoid(y_pred).squeeze(1).detach()
...
```{python}
save_name = f'{MODEL}_fastai_{EPOCHS}_{LR}_{WD}'
save_name += f'_{getNextFilePath(MODELS_PATH, save_name)}'
log_dir=Path('/work/stages/schwob/runs/')
```
...
net = Net(mod, opt, nn.BCEWithLogitsLoss(), [mean_iou], cfg.MODELS_PATH)
```
